...
function majDonneesPanier() {
    if (panier.length > 0) {
        ...
        $('#shipping').html(prixTotal * 0.05 + " <i class='fab fa-btc'></i>");
        $('#taxe').html(prixTotal * 0.1 + " <i class='fab fa-btc'></i>");
        var total = (prixTotal * 0.1) + (prixTotal * 0.05) + prixTotal;
...
function menu() {
    ...
    for (var i = 0; i < zone.length; i++) {
        ...
        for (var j = 0; j < tableauProduits.length; j++) {
            if (zone[i] == (tableauProduits[j].getZone())) {
